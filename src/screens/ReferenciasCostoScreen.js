...
const fleteAsignadoValue = fleteAsignado.trim() === '' ? 0 : parseDecimalInput(fleteAsignado);
...
return (
<AppShell title="Referencias costo" subtitle="Trazabilidad mínima de costo para decisión comercial" description="Supervisor prepara, admin revisa y aplica cuando corresponda." layout="top" >
  ...
  {canCreate ? (
    <View style={styles.card}>
      ...
      <ScrollView style={styles.selectorList} nestedScrollEnabled>
        ...
        {inventarioItems.map((item) => (
          ...
          >
            ...
            <Text style={styles.selectorOptionTitle}>{item.productoNombre} ({item.unidadBase || 'sin unidad'})</Text>
      ...
      <Text style={styles.helperText}>
        ...
        Seleccionado: {selectedInventario ? `${selectedInventario.productoNombre} (${selectedInventario.unidadBase || 'sin unidad'})` : 'ninguno'}
...
>
          ...
          <Text style={styles.cardRowTitle}>{item.productoNombre} ({item.unidadBase || 'sin unidad'})</Text>
